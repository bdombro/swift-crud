...
enum AuthCookie {
    ...
    static func verify(_ cookieValue: String, secret: String) -> Int? {
        ...
        guard let sigData = Data(base64Encoded: String(parts[1])),
              sigData.count == SHA256.byteCount else { return nil }
...
struct HTTPRequest {
    ...
    var queryParameters: [String: String] {
        ...
        return Dictionary(uniqueKeysWithValues: query.split(separator: "&").compactMap { pair in
...
struct HTTPResponse {
    ...
    static func json<T: Encodable>(_ status: HTTPResponseStatus, _ payload: T) -> HTTPResponse {
        ...
        var headers: HTTPHeaders = [HTTPHeader("Content-Type"): "application/json"]
